...
import { rejectKnock } from '@command-center/api/store'
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> | { id: string } }) {
            ...
            await payload.update({
                ...
                data: {
                    event: {
                        ...prevEvent,
                        status: 'failed',
                        detail: `Rejected by ${me.email ?? 'admin'}.`,
                    },
                },
            })
        }
    ...
    return NextResponse.json({ id: knock.id, status: 'rejected', message: 'Knock rejected.' })
}
